...
```{r simple model (Exp1), echo=T, message=F, warning=F, eval=T}
...
```{r simple model (Exp2), echo=T, message=F, warning=F, eval=T}
# Model fitting: fixed-effects & mixed-effects
# m1 <- glm(correct ~ macron*c.(familiarity) + c.(score) + c.(n.neighbors) + c.(length), data=dataExp2, family=binomial(link="logit"))
# summary(m1) # remove score
# m2 <- update(m1, . ~ . -c.(score))
# anova(m1, m2, test="Chisq") # not significant, choose m2 (remove score)
# m3 <- glmer(correct ~ macron*c.(familiarity) + c.(score) + c.(n.neighbors) + c.(length) + (1 + macron*c.(familiarity) + c.(score) + c.(n.neighbors) + c.(length)|workerId) + (1|word), family = binomial(link = "logit"), data = dataExp2, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e6)))
# summary(m3)
```
