...
The data is structured as follows:
  ...
  - <span style="color:blue">*enteredResponse*</span> is the wellformedness rating for each stimulus.
...
### Comparing the AIC score of four statistical models with measures for evaluationg participants

```{r AIC (Exp1), echo=F, message=F, warning=F, eval=T}
...
tibble(
  "Evaluation measure"=c("Dprime",
                          "Basic knowledge",
                          "Proficiency",
                          "Exposure"),
  "AIC" = c(AIC(mExp1Dprime),
          AIC(mExp1MaoriList),
          AIC(mExp1MaoriProf),
          AIC(mExp1MaoriExpo))
) %>%
display_table(digits=1, highlight=c("AIC"), caption="Comparison of the AIC score (Experiment 1)")
```
...
```{r accuracy rate, echo=F, message=F, warning=F, eval=T, fig.width=8, fig.height=6, fig.cap="Rate of accurate definitions per word. Words are displayed according to their phonotactic score on the x-axis and their accuracy rates are represented on the y-axis. Overlapping labels are not shown."}
Fig6 <- dataExp2[,c("workerId","word","correct","familiarity","score")]
...
wordScore <- unique(Fig6[,c("word","familiarity","score")])
accRate <- merge(accRate, wordScore, by="word")
ggplot(accRate, aes(x=score, y=rate, label=word)) + labs(x= "Phonotactic score", y="Accuracy rate") + geom_point(color="blue", size=3, alpha=0.2, shape=17) + theme(legend.title=element_blank()) + geom_text(color="blue", hjust=0.2,vjust=-0.7,check_overlap = TRUE, size=4) + theme_bw() + theme(legend.position="none", legend.title=element_blank(),panel.background = element_blank(), axis.text=element_text(size=13), axis.title=element_text(size = 13, face = "bold"))
```
...
```{r AIC (Exp2), echo=F, message=F, warning=F, eval=T}
mExp2MaoriList <- readRDS("./exp2MaoriList.rds")
mExp2MaoriProf <- readRDS("./exp2MaoriProf.rds")
mExp2MaoriExpo <- readRDS("./exp2MaoriExpo.rds")
tibble(
  "Evaluation measure"=c("Basic knowledge",
                          "Proficiency",
                          "Exposure"),
  "AIC" = c(AIC(mExp2MaoriList),
          AIC(mExp2MaoriProf),
          AIC(mExp2MaoriExpo))
) %>%
display_table(digits=1, highlight=c("AIC"), caption="Comparison of the AIC score (Experiment 2)")
```
...
exp2ML_1$type <- "Word and\nNonword"
F_Exp2ML_1 <- ggplot(exp2ML_1, aes(x=familiarity, y=fit, color = type, fill=type)) + geom_line(size=1) + facet_grid(. ~ maoriList1) + geom_ribbon(aes(ymin=fit-se, ymax=fit+se),alpha=0.2, color=NA) + labs(x= "Familiarity", y="Accuracy") + scale_color_manual(values = c("Word and\nNonword" = "purple")) + scale_fill_manual(values = c("Word and\nNonword" = "purple")) + theme_bw() + theme(legend.position="none", legend.title=element_blank(), panel.background = element_blank(), axis.text=element_text(size=10), axis.title=element_text(size = 12, face = "bold"),legend.text=element_text(size=10), strip.text = element_text(size = 12, face="bold")) 
...
exp2MP_1$type <- "Word and\nNonword"
F_Exp2MP_1 <- ggplot(exp2MP_1, aes(x=familiarity, y=fit, color = type, fill=type)) + geom_line(size=1) + facet_grid(. ~ maoriProf1) + geom_ribbon(aes(ymin=fit-se, ymax=fit+se),alpha=0.2, color=NA) + labs(x= "Familiarity", y="Accuracy") + scale_color_manual(values = c("Word and\nNonword" = "purple")) + scale_fill_manual(values = c("Word and\nNonword" = "purple")) + theme_bw() + theme(legend.position="none", legend.title=element_blank(), panel.background = element_blank(), axis.text=element_text(size=10), axis.title=element_text(size = 12, face = "bold"),legend.text=element_text(size=10), strip.text = element_text(size = 12, face="bold"))
...
exp2ME_1$type <- "Word and\nNonword"
exp2ME_1$maoriExpo1 <- factor(exp2ME_1$maoriExpo1, levels=c("Exposure = 2","Exposure = 4","Exposure = 6","Exposure = 8","Exposure = 10"))
F_Exp2ME_1 <- ggplot(exp2ME_1, aes(x=n.neighbors, y=fit, color = type, fill=type)) + geom_line(size=1) + facet_grid(. ~ maoriExpo1) + geom_ribbon(aes(ymin=fit-se, ymax=fit+se),alpha=0.2, color=NA) + labs(x= "Neighbourhood density", y="Accuracy") + scale_color_manual(values = c("Word and\nNonword" = "purple")) + scale_fill_manual(values = c("Word and\nNonword" = "purple")) + theme_bw() + theme(legend.position="none", legend.title=element_blank(), panel.background = element_blank(), axis.text=element_text(size=9), axis.title=element_text(size = 9, face = "bold"),legend.text=element_text(size=9), strip.text = element_text(size = 9, face="bold"))
...
exp2ME_2$type <- "Word and\nNonword"
F_Exp2ME_2 <- ggplot(exp2ME_2, aes(x=macron, y=fit, color=type, fill=type)) + geom_point(size=4, position=position_dodge(width=0.2)) + geom_line(aes(x=as.integer(macron)), size=1, alpha=0.4, position=position_dodge(width=0.2)) + geom_errorbar(aes(ymin=fit-se, ymax=fit+se), size=1, width=0.3, position=position_dodge(width=0.2)) + xlab("Presence of macron") + ylab("Accuracy") + ylim(c(0, 1)) + scale_color_manual(values = c("Word and\nNonword" = "purple")) + scale_fill_manual(values = c("Word and\nNonword" = "purple")) + theme_bw() + theme(plot.title = element_text(hjust = 0.5, size=9), legend.position="none", legend.title=element_blank(), panel.background = element_blank(), axis.text=element_text(size=9), axis.title=element_text(size = 9, face = "bold"),legend.text=element_text(size=9), strip.text = element_text(size = 9, face="bold"))

# 3) Familiarity
exp2ME_3 <- as.data.frame(effect(term="c.(familiarity)", mod=mExp2MaoriExpo))
exp2ME_3$type <- "Word and\nNonword"
F_Exp2ME_3 <- ggplot(exp2ME_3, aes(x=familiarity, y=fit, color = type, fill=type)) + geom_line(size=1) + geom_ribbon(aes(ymin=fit-se, ymax=fit+se),alpha=0.2, color=NA) + labs(x= "Familiarity", y="Accuracy") + scale_color_manual(values = c("Word and\nNonword" = "purple")) + scale_fill_manual(values = c("Word and\nNonword" = "purple")) + theme_bw() + theme(legend.position="right", legend.title=element_blank(), panel.background = element_blank(), axis.text=element_text(size=10), axis.title=element_text(size = 10, face = "bold"),legend.text=element_text(size=10), strip.text = element_text(size = 10, face="bold"))
...
```{r Figure 6, echo=F, message=F, warning=F, eval=T, fig.width=8, fig.height=6, fig.cap="Figure 6: Rate of accurate definitions per word. Words are displayed according to their phonotactic score on the x-axis and their accuracy rates are represented on the y-axis. Overlapping labels are not shown."}
Fig6 <- dataExp2[,c("workerId","word","correct","familiarity","score")]
Fig6$correct1 <- 0;Fig6[Fig6$correct=="TRUE",]$correct1 <- 1;Fig6$correct1 <- as.numeric(Fig6$correct1)
accRate <- aggregate(Fig6$correct1, by=list(Fig6$word),  FUN=sum)
colnames(accRate) <- c("word","nbCorrect")
