...
# m11 <- glmer(correct ~ c.(familiarity) + c.(n.neighbors)*macron + (1 + c.(familiarity) + macron|workerId) + (1|word), family = binomial(link = "logit"), data = dataExp2, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e6)))
# anova(m10, m11) # not significant, choose m11 (remove length)
# summary(m11) # remove n.neighbors:macron
# m12 <- glmer(correct ~ c.(familiarity) + c.(n.neighbors) + macron + (1 + c.(familiarity) + macron|workerId) + (1|word), family = binomial(link = "logit"), data = dataExp2, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e6)))
# anova(m11, m12) # not significant, choose m12 (remove n.neighbors:macron)
...
# anova(m12, m13) # not significant, choose m13 (remove n.neighbors)
# summary(m13) # AIC: 12288.7, remove familiarity:maoriList
...
# summary(m7) # singular fit, remove length
...
# summary(m8) # singular fit, remove n.neighbors
...
# anova(m8, m9) # not significant, choose m9 (remove n.neighbors)
# summary(m9) # remove familiarity:maoriProf
...
# summary(m12) # AIC: 12476.1, remove n.neighbors:maoriExpo
...
```{r Figure 7, echo=F, message=F, warning=F, eval=T, fig.width=7, fig.height=2.5, fig.cap="Figure 7: Effect plots of: familiarity (Fig.a); and the presence of macrons (Fig.b)."}
# 1) Familiarity
exp2_1 <- as.data.frame(effect(term="c.(familiarity)", mod=mExp2))
exp2_1$type <- "Word"
F_Exp2_1 <- ggplot(exp2_1, aes(x=familiarity, y=fit, color = type, fill=type)) + geom_line(size=1) + geom_ribbon(aes(ymin=fit-se, ymax=fit+se),alpha=0.2, color=NA) + labs(x= "Familiarity", y="Accuracy") + scale_color_manual(values = c("Word" = "blue")) + scale_fill_manual(values = c("Word" = "blue")) + theme_bw() + theme(legend.position="none", legend.title=element_blank(), panel.background = element_blank(), axis.text=element_text(size=10), axis.title=element_text(size = 10, face = "bold"),legend.text=element_text(size=10), strip.text = element_text(size = 10, face="bold"))
