...
```{r Fig.5, echo=FALSE, message=FALSE, warning=FALSE, fig.width=11, fig.height=9, fig.cap="Figure 5: Effect plots of the interaction between the presence of macrons and the distinction between non vs. real word stimuli on the top (Fig.5a), the interaction between the neighbourhood density and the distinction between non vs. real word stimuli on the middle (Fig.5b), the interaction between d′ and the presence of macrons (Fig.5c), and the frequency-weighted dictionary phonotactic score on the bottom (Fig.5d). Plots on the left show predicted mean ratings and plots on the right show predicted distributions over ratings."}

# 1) Type*Macron
Figure5_mean_1 <- clm_plotdat(modelTable1, c("type", "macron"), xlevels=25, type="mean") %>%
  mutate(
...
Figure5_dist_1 <- clm_plotdat(modelTable1, c("type", "macron"), xlevels=25, type="dist") %>%
  mutate(
    type = fct_recode(type, "Nonword"="non", "Word"="real") %>% fct_relevel("Word"),
    macron.int = as.integer(macron),
    macron = fct_recode(macron, "Yes"="TRUE", "No"="FALSE")
...
# 2) Type*Neighbourhood density
Figure5_mean_2 <- clm_plotdat(modelTable1, c("type", "n.neighbors"), xlevels=list(length=25), type="mean") %>%
  mutate(
    type = fct_recode(type, "Nonword"="non", "Word"="real") %>% fct_relevel("Word")
  ) %>%
...
Figure5_dist_2 <- clm_plotdat(modelTable1, c("type", "n.neighbors"), xlevels=list(length=25), type="dist") %>%
  mutate(
    type = fct_recode(type, "Nonword"="non", "Word"="real") %>% fct_relevel("Word")
  ) %>%
  ggplot(., aes(x=n.neighbors, y=pred, color=type, fill=type)) +
...
# 3) Dprime*Macron
Figure5_mean_3 <- clm_plotdat(modelTable1, c("dprime", "macron"), xlevels=list(dprime=c(1.5805, 2.453)), type="mean") %>%
  mutate(
    dprime = factor(dprime, labels=c("LQ (1.5805)", "UQ (2.453)")) %>% fct_relevel("UQ (2.453)"),
    macron.int = as.integer(macron),
    macron = fct_recode(macron, "Yes"="TRUE", "No"="FALSE")
  ) %>%
  ggplot(., aes(x=macron, y=pred, color=dprime, shape=dprime)) +
  geom_point(size=4, position=position_dodge(width=0.2)) +
  geom_line(aes(x=as.integer(macron)), size=1, alpha=0.4, position=position_dodge(width=0.2)) +
  geom_errorbar(aes(ymin=lci, ymax=uci), size=1, width=0.3, position=position_dodge(width=0.2)) +
  xlab("Presence of macron") +
...
Figure5_dist_3 <- clm_plotdat(modelTable1, c("dprime", "macron"), xlevels=list(dprime=c(1.5805, 2.453)), type="dist") %>%
  mutate(
    dprime = factor(dprime, labels=c("LQ (1.5805)", "UQ (2.453)")) %>% fct_relevel("UQ (2.453)"),
    macron.int = as.integer(macron),
    macron = fct_recode(macron, "Yes"="TRUE", "No"="FALSE")
...
ggarrange(Figure5_mean_1, Figure5_dist_1,Figure5_mean_2,Figure5_dist_2, Figure5_mean_3,Figure5_dist_3, Figure5_mean_4, Figure5_dist_4, labels=c("a.", "","b.","","c.","","d.",""),ncol=2, widths=c(1,6))
```

## TableS1: Modeling confidence ratings with an ordinal mixed effects model with a subset of data
...
```{r Fig.S1, echo=FALSE, message=FALSE, warning=FALSE, fig.width=11, fig.height=4.5, fig.cap="Figure S1: Effect plots of phonotactic score (Fig.S1a) and neighbourhood density (Fig.S1b). Plots on the left show predicted mean ratings and plots on the right show predicted distributions over ratings."}
# 1) Phonotactic score
FigureS1_mean_1 <- clm_plotdat(modelA, "scoreDictToken", xlevels=25, type="mean") %>%
  mutate(type="Word and\nNonword") %>%
  ggplot(., aes(x=scoreDictToken, y=pred, color=type, fill=type)) +
  geom_ribbon(aes(ymin=lci, ymax=uci), alpha=0.3, color=NA) +
  geom_line(size=1) +
...
FigureS1_dist_1 <- clm_plotdat(modelA, "scoreDictToken", xlevels=25, type="dist") %>%
  mutate(type="Word and\nNonword") %>%
  ggplot(., aes(x=scoreDictToken, y=pred, color=type, fill=type)) +
  geom_ribbon(aes(ymin=lci, ymax=uci), alpha=0.3, color=NA) +
  geom_line(size=1) +
...
FigureS1_mean_2 <- clm_plotdat(modelA, "n.neighbors", xlevels=25, type="mean") %>%
  mutate(type="Word and\nNonword") %>%
  ggplot(., aes(x=n.neighbors, y=pred, color=type, fill=type)) +
  geom_ribbon(aes(ymin=lci, ymax=uci), alpha=0.3, color=NA) +
  geom_line(size=1) +
...
FigureS1_dist_2 <- clm_plotdat(modelA, "n.neighbors", xlevels=25, type="dist") %>%
  mutate(type="Word and\nNonword") %>%
  ggplot(., aes(x=n.neighbors, y=pred, color=type, fill=type)) +
  geom_ribbon(aes(ymin=lci, ymax=uci), alpha=0.3, color=NA) +
  geom_line(size=1) +
...
ggarrange(FigureS1_mean_1, FigureS1_dist_1, FigureS1_mean_2, FigureS1_dist_2, labels=c("a.", "","b.",""),ncol=2, widths=c(1,6))
``` 
...
Fig7 <- as.data.frame(effect(term="c.(scoreRsSeg)", mod=modelTable2))
ggplot(Fig7, aes(x=scoreRsSeg, y=fit)) + geom_point(aes(color="#00BFC4")) + geom_line(aes(color="#00BFC4")) + geom_ribbon(aes(ymin=lower, ymax=upper, fill="#00BFC4"),alpha=0.1) + labs(x= "Phonotactic score", y="Predicted accuracy") + theme_classic()+ guides(fill=FALSE, color=FALSE)
```
...
Fig8 <- as.data.frame(effect(term="c.(n.neighbors)", mod=modelFigure8))
ggplot(Fig8, aes(x=n.neighbors, y=fit)) + geom_point(aes(color="#00BFC4")) + geom_line(aes(color="#00BFC4")) + geom_ribbon(aes(ymin=lower, ymax=upper, fill="#00BFC4"),alpha=0.1) + labs(x= "Neighbourhood density", y="Predicted accuracy") + theme_classic()+ guides(fill=FALSE, color=FALSE)
```
